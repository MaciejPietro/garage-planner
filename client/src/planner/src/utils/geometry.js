...
export function checkCrossProduct(linePoint1, linePoint2, currentPoint){
  const v1 = [linePoint2.x - linePoint1.x, linePoint2.y -linePoint1.y];
  const v2 = [linePoint2.x - currentPoint.x, linePoint2.y - currentPoint.y]; // # Vector 1
  const xp = v1[0]*v2[1] - v1[1]*v2[0]  // Cross product
...
export function calculatePointFromLineWithAngleAndPoint(startingLinePointX, startingLinePointY, lineAngle, currentPointX, currentPointY) {
  ...
  const extremeMaxPoint = findPointOnLineByPointAndAngle(startingLinePointX, startingLinePointY, lineAngle, 10000);
  const extremeMinPoint = findPointOnLineByPointAndAngle(startingLinePointX, startingLinePointY, lineAngle, -10000);

  const A = new Vector3(currentPointX, currentPointY, 0);
  const B = new Vector3(extremeMaxPoint.x, extremeMaxPoint.y, 0); // line base point 1
  const C = new Vector3(extremeMinPoint.x, extremeMinPoint.y, 0); // line base point 2

  const line = new Line3(B, C);
  const distance = line.closestPointToPoint(A).distanceTo(A);

  const checkCross = checkCrossProduct(
      {x: extremeMinPoint.x, y: extremeMinPoint.y},
      {x: extremeMaxPoint.x, y: extremeMaxPoint.y},
