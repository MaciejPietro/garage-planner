...
import { SELECT_TOOL_DRAWING_ITEM, UPDATE_DRAWING_ITEM, END_DRAWING_ITEM, BEGIN_DRAGGING_ITEM, UPDATE_DRAGGING_ITEM, END_DRAGGING_ITEM, BEGIN_ROTATING_ITEM, UPDATE_ROTATING_ITEM, END_ROTATING_ITEM, BEGIN_RESIZING_ITEM, UPDATE_RESIZING_ITEM, END_RESIZING_ITEM, SELECT_ITEM } from '../constants';
...
export default function (state, action) {
  switch (action.type) {
    ...
    case BEGIN_RESIZING_ITEM:
      state = state.merge({ sceneHistory: history.historyPush(state.sceneHistory, state.scene) });
      return Item.beginResizingItem(state, action.layerID, action.itemID, action.x, action.y, action.side).updatedState;

    case UPDATE_RESIZING_ITEM:
      return Item.updateResizingItem(state, action.x, action.y).updatedState;

    case END_RESIZING_ITEM:
      state = state.merge({ sceneHistory: history.historyPush(state.sceneHistory, state.scene) });
      return Item.endResizingItem(state, action.x, action.y).updatedState;
