...
import { Project, Line, Hole, Item, Camera, Camera360, Area, Layer, Vertex } from './export';
    ...
    _createClass(Group, null, [{
        ...
        value: function select(state, groupID) {
            ...
            layerList.entrySeq().forEach(function (_ref) {
                ...
                if (cameras) cameras.forEach(function (cameraID) {
                    state = Camera.select(state, groupLayerID, cameraID).updatedState;
                });
                if (cameras360) cameras360.forEach(function (camera360ID) {
                    state = Camera360.select(state, groupLayerID, camera360ID).updatedState;
                });
        ...
        value: function removeAndDeleteElements(state, groupID) {
            ...
            layerList.entrySeq().forEach(function (_ref5) {
                ...
                if (cameras) cameras.forEach(function (cameraID) {
                    state = Camera.remove(state, groupLayerID, cameraID).updatedState;
                });
                if (cameras360) cameras360.forEach(function (camera360ID) {
                    state = Camera360.remove(state, groupLayerID, camera360ID).updatedState;
                });
        ...
        value: function translate(state, groupID, x, y) {
            ...
            layerList.entrySeq().forEach(function (_ref7) {
                ...
                if (cameras) state = cameras.map(function (cameraID) {
                    ...
                    return Camera.setAttributes(newState, groupLayerID, camera.id, new Map({ x: xI + deltaX, y: yI + deltaY })).updatedState;
                }, state);
                ...
                if (cameras360) state = cameras360.map(function (camera360ID) {
                    ...
                    return Camera360.setAttributes(newState, groupLayerID, camera360.id, new Map({ x: xI + deltaX, y: yI + deltaY })).updatedState;
                }, state);
        ...
        value: function rotate(state, groupID, newAlpha) {
            ...
            layerList.entrySeq().forEach(function (_ref9) {
                ...
                if (cameras) state = cameras.map(function (cameraID) {
                    ...
                    var _GeometryUtils$rotate3 = GeometryUtils.rotatePointAroundPoint(xI, yI, barX, barY, alpha),
                        newX = _GeometryUtils$rotate3.x,
                        newY = _GeometryUtils$rotate3.y;

                    return Camera.setAttributes(newState, groupLayerID, camera.id, new Map({ x: newX, y: newY, rotation: rI + alpha })).updatedState;
                }, state);
                ...
                if (cameras360) state = cameras360.map(function (camera360ID) {
                    ...
                    var _GeometryUtils$rotate4 = GeometryUtils.rotatePointAroundPoint(xI, yI, barX, barY, alpha),
                        newX = _GeometryUtils$rotate4.x,
                        newY = _GeometryUtils$rotate4.y;

                    return Camera360.setAttributes(newState, groupLayerID, camera360.id, new Map({ x: newX, y: newY, rotation: rI + alpha })).updatedState;
                }, state);
