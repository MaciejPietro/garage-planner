...
import { Project, Area, Line, Hole, Item, Camera, Camera360, Vertex } from './export';
    ...
    _createClass(Layer, null, [{
        ...
        value: function unselectAll(state, layerID) {
            var _state$getIn = state.getIn(['scene', 'layers', layerID]),
                ...
                cameras = _state$getIn.cameras,
                cameras360 = _state$getIn.cameras360,
            ...
            if (cameras) cameras.forEach(function (camera) {
                state = Camera.unselect(state, layerID, camera.id).updatedState;
            });
            if (cameras360) cameras360.forEach(function (camera360) {
                state = Camera360.unselect(state, layerID, camera360.id).updatedState;
            });
        ...
        value: function setPropertiesOnSelected(state, layerID, properties) {
            ...
            selected.cameras.forEach(function (cameraID) {
                return state = Camera.setProperties(state, layerID, cameraID, properties).updatedState;
            });
            selected.cameras360.forEach(function (camera360ID) {
                return state = Camera360.setProperties(state, layerID, camera360ID, properties).updatedState;
            });
        ...
        value: function updatePropertiesOnSelected(state, layerID, properties) {
            ...
            selected.cameras.forEach(function (cameraID) {
                return state = Camera.updateProperties(state, layerID, cameraID, properties).updatedState;
            });
            selected.cameras360.forEach(function (camera360ID) {
                return state = Camera360.updateProperties(state, layerID, camera360ID, properties).updatedState;
            });
        ...
        value: function setAttributesOnSelected(state, layerID, attributes) {
            ...
            selected.cameras.forEach(function (cameraID) {
                return state = Camera.setAttributes(state, layerID, cameraID, attributes).updatedState;
            });
            selected.cameras360.forEach(function (camera360ID) {
                return state = Camera360.setAttributes(state, layerID, camera360ID, attributes).updatedState;
            });
