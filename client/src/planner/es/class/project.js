...
import { MODE_VIEWING_CATALOG, MODE_IDLE, MODE_CONFIGURING_PROJECT } from '../constants';
import { State, Catalog } from '../models';
import { history } from '../utils/export';
import { Layer, Group, Line, Hole, Item, Camera, Camera360, HorizontalGuide, VerticalGuide } from '../class/export';
...
var Project = function () {
    ...
    _createClass(Project, null, [{
        ...
        key: 'setCamerasAttributes',
        value: function setCamerasAttributes(state, attributes) {
            //TODO apply only to items
            state.getIn(['scene', 'layers']).forEach(function (layer) {
                state = Layer.setAttributesOnSelected(state, layer.id, attributes).updatedState;
            });
        ...
        key: 'setCameras360Attributes',
        value: function setCameras360Attributes(state, attributes) {
            //TODO apply only to items
            state.getIn(['scene', 'layers']).forEach(function (layer) {
                state = Layer.setAttributesOnSelected(state, layer.id, attributes).updatedState;
            });
        ...
        value: function remove(state) {
            ...
            var _state$getIn = state.getIn(['scene', 'layers', selectedLayer, 'selected']),
                ...
                selectedCameras = _state$getIn.cameras,
                selectedCameras360 = _state$getIn.cameras360;
            ...
            selectedCameras.forEach(function (cameraID) {
                state = Camera.remove(state, selectedLayer, cameraID).updatedState;
            });
            selectedCameras360.forEach(function (camera360ID) {
                state = Camera360.remove(state, selectedLayer, camera360ID).updatedState;
            });
        ...
        key: 'setSceneElementsHeight',
        value: function setSceneElementsHeight(state, height) {
            state = state.set('elelementsHeight', height);
            return { updatedState: state };
        }
    }]);
