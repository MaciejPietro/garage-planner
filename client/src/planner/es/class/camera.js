...
import { MODE_IDLE, MODE_DRAWING_CAMERA, MODE_DRAGGING_CAMERA, MODE_ROTATING_CAMERA, MODE_RESIZING_CAMERA } from '../constants';
...
var Camera = function () {
  ...
  _createClass(Camera, null, [{
    ...
    value: function create(state, layerID, type, x, y, rotation) {
      var cameraID = IDBroker.acquireID();
      console.log('camera-create');
      var camera = state.catalog.factoryElement(type, {
        id: cameraID,
        name: NameGenerator.generateName('cameras', state.catalog.getIn(['elements', type, 'info', 'title'])),
        type: type,
        x: x,
        y: y,
        rotation: rotation
    ...
    value: function remove(state, layerID, cameraID) {
      ...
      state.getIn(['scene', 'groups']).forEach(function (group) {
        return state = Group.removeElement(state, group.id, layerID, 'cameras', cameraID).updatedState;
      });
    ...
    value: function updateResizingCamera(state, x, y) {
      ...
      var mousePositionToCameraCenter = calculatePointFromLineWithAngleAndPoint(originalX, originalY, sideAngle, x, y);
