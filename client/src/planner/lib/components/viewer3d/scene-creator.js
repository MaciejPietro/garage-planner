...
var _camerasActions = require('../../actions/cameras-actions');

var _cameras360Actions = require('../../actions/cameras360-actions');
...
function createLayerObjects(layer, planData, sceneData, actions, catalog) {
    ...
    // Import cameras
    layer.cameras.forEach(function (camera) {
        promises.push(addCamera(sceneData, planData, layer, camera.id, catalog, actions.camerasActions));
    });

    // Import cameras
    layer.cameras360.forEach(function (camera360) {
        promises.push(addCamera360(sceneData, planData, layer, camera360.id, catalog, actions.cameras360Actions));
    });
        ...
        if (path[1] === 'layers') {
            ...
            if (layerGraph) {
                if (!layerGraph.visible) {
                    ...
                    }for (var cameraID in layerGraph.cameras) {
                        removeCamera(planData, layerGraph.id, cameraID);
                    }for (var camera360ID in layerGraph.cameras360) {
                        removeCamera360(planData, layerGraph.id, camera360ID);
...
function replaceObject(modifiedPath, layer, planData, actions, sceneData, oldSceneData, catalog) {
    ...
    switch (modifiedPath[3]) {
        ...
        case 'items':
            ...
            break;
        case 'cameras':
            var camera = layer.getIn(['cameras', modifiedPath[4]]);

            if (catalog.getElement(camera.type).updateRender3D) {
                promises.push(updateCamera(sceneData, oldSceneData, planData, layer, modifiedPath[4], modifiedPath.slice(5), catalog, actions.camerasActions, function () {
                    return removeCamera(planData, layer.id, modifiedPath[4]);
                }, function () {
                    return addCamera(sceneData, planData, layer, modifiedPath[4], catalog, actions.camerasActions);
                }));
            } else {
                removeCamera(planData, layer.id, modifiedPath[4]);
                promises.push(addCamera(sceneData, planData, layer, modifiedPath[4], catalog, actions.camerasActions));
            }
            break;

        case 'cameras360':
            var camera360 = layer.getIn(['cameras360', modifiedPath[4]]);

            if (catalog.getElement(camera360.type).updateRender3D) {
                promises.push(updateCamera360(sceneData, oldSceneData, planData, layer, modifiedPath[4], modifiedPath.slice(5), catalog, actions.cameras360Actions, function () {
                    return removeCamera360(planData, layer.id, modifiedPath[4]);
                }, function () {
                    return addCamera360(sceneData, planData, layer, modifiedPath[4], catalog, actions.cameras360Actions);
                }));
            } else {
                removeCamera360(planData, layer.id, modifiedPath[4]);
                promises.push(addCamera360(sceneData, planData, layer, modifiedPath[4], catalog, actions.cameras360Actions));
            }
            break;
        ...
        case 'visible':
            if (!layer.visible) {
                ...
                }for (var cameraID in _layerGraph.cameras) {
                    removeCamera(planData, layer.id, cameraID);
                }for (var camera360ID in _layerGraph.cameras360) {
                    removeCamera360(planData, layer.id, camera360ID);
        ...
        case 'altitude':
            ...
            }for (var _cameraID in layerGraph.cameras) {
                removeCamera(planData, layer.id, _cameraID);
            }for (var _camera360ID in layerGraph.cameras360) {
                removeCamera360(planData, layer.id, _camera360ID);
...
function removeObject(modifiedPath, layer, planData, actions, sceneData, oldSceneData, catalog) {
    ...
    switch (modifiedPath[3]) {
        ...
        case 'cameras':
            if (modifiedPath.length === 5) {
                // I am removing an camera
                removeCamera(planData, layer.id, modifiedPath[4]);
            }
            break;
        case 'cameras360':
            if (modifiedPath.length === 5) {
                // I am removing an camera360
                removeCamera360(planData, layer.id, modifiedPath[4]);
            }
            break;
    }
...
function removeLayer(layerId, planData) {
    ...
    for (var lineID in layerGraph.lines) {
        ...
        removeCamera(planData, layerId, cameraID);
    }for (var camera360ID in layerGraph.cameras360) {
        removeCamera360(planData, layerId, camera360ID);
    }for (var holeID in layerGraph.holes) {
...
function removeCamera(planData, layerId, cameraID) {
    if (planData.sceneGraph.busyResources.layers[layerId].cameras[cameraID]) {
        setTimeout(function () {
            return removeCamera(planData, layerId, cameraID);
        }, 100);
        return;
    }

    planData.sceneGraph.busyResources.layers[layerId].cameras[cameraID] = true;

    var camera3D = planData.sceneGraph.layers[layerId].cameras[cameraID];

    if (camera3D) {
        planData.plan.remove(camera3D);
        (0, _threeMemoryCleaner.disposeObject)(camera3D);
        delete planData.sceneGraph.layers[layerId].cameras[cameraID];
        delete planData.sceneGraph.LODs[cameraID];
        camera3D = null;
        updateBoundingBox(planData);
    }

    planData.sceneGraph.busyResources.layers[layerId].cameras[cameraID] = false;
}

function removeCamera360(planData, layerId, camera360ID) {
    if (planData.sceneGraph.busyResources.layers[layerId].cameras360[camera360ID]) {
        setTimeout(function () {
            return removeCamera360(planData, layerId, camera360ID);
        }, 100);
        return;
    }

    planData.sceneGraph.busyResources.layers[layerId].cameras360[camera360ID] = true;

    var camera3603D = planData.sceneGraph.layers[layerId].cameras360[camera360ID];

    if (camera3603D) {
        planData.plan.remove(camera3603D);
        (0, _threeMemoryCleaner.disposeObject)(camera3603D);
        delete planData.sceneGraph.layers[layerId].cameras360[camera360ID];
        delete planData.sceneGraph.LODs[camera360ID];
        camera3603D = null;
        updateBoundingBox(planData);
    }

    planData.sceneGraph.busyResources.layers[layerId].cameras360[camera360ID] = false;
}
    ...
    if (modifiedPath.length === 5) {
        ...
        switch (modifiedPath[3]) {
            ...
            case 'items':
                ...
                break;
            case 'cameras':
                addPromise = addCamera;
                addAction = actions.camerasActions;
                break;
            case 'cameras360':
                addPromise = addCamera360;
                addAction = actions.cameras360Actions;
                break;
            case 'holes':
                addPromise = addHole;
...
function addCamera(sceneData, planData, layer, cameraID, catalog, camerasActions) {
    var camera = layer.getIn(['cameras', cameraID]);

    return catalog.getElement(camera.type).render3D(camera, layer, sceneData).then(function (camera3D) {
        if (camera3D instanceof Three.LOD) {
            planData.sceneGraph.LODs[cameraID] = camera3D;
        }
        ...
        pivot.rotation.y = camera.rotation * Math.PI / 180;
        pivot.position.x = camera.x;
        pivot.position.y = layer.altitude;
        pivot.position.z = -camera.y;

        applyInteract(camera3D, function () {
            console.log(camerasActions);

            camerasActions.selectCamera(layer.id, camera.id);
        });
        ...
        planData.plan.add(pivot);
        planData.sceneGraph.layers[layer.id].cameras[camera.id] = pivot;
    });
}

function addCamera360(sceneData, planData, layer, camera360ID, catalog, cameras360Actions) {
    var camera360 = layer.getIn(['cameras360', camera360ID]);

    return catalog.getElement(camera360.type).render3D(camera360, layer, sceneData).then(function (camera3603D) {
        if (camera3603D instanceof Three.LOD) {
            planData.sceneGraph.LODs[camera360ID] = camera3603D;
        }

        var pivot = new Three.Object3D();
        pivot.name = 'pivot';
        pivot.add(camera3603D);

        pivot.rotation.y = camera360.rotation * Math.PI / 180;
        pivot.position.x = camera360.x;
        pivot.position.y = layer.altitude;
        pivot.position.z = -camera360.y;

        applyInteract(camera3603D, function () {
            cameras360Actions.selectCamera360(layer.id, camera360.id);
        });

        var opacity = layer.opacity;
        if (camera360.selected) {
            opacity = 1;
        }
        ...
        planData.plan.add(pivot);
        planData.sceneGraph.layers[layer.id].cameras360[camera360.id] = pivot;
    });
}

function updateCamera(sceneData, oldSceneData, planData, layer, cameraID, differences, catalog, camerasActions, selfDestroy, selfBuild) {
    var camera = layer.getIn(['cameras', cameraID]);
    var oldCamera = oldSceneData.getIn(['layers', layer.id, 'cameras', cameraID]);
    var mesh = planData.sceneGraph.layers[layer.id].cameras[cameraID];

    if (!mesh) return null;

    return catalog.getElement(camera.type).updateRender3D(camera, layer, sceneData, mesh, oldCamera, differences, selfDestroy, selfBuild);
}

function updateCamera360(sceneData, oldSceneData, planData, layer, camera360ID, differences, catalog, cameras360Actions, selfDestroy, selfBuild) {
    var camera360 = layer.getIn(['cameras360', camera360ID]);
    var oldCamera360 = oldSceneData.getIn(['layers', layer.id, 'cameras360', camera360ID]);
    var mesh = planData.sceneGraph.layers[layer.id].cameras360[camera360ID];

    if (!mesh) return null;

    return catalog.getElement(camera360.type).updateRender3D(camera360, layer, sceneData, mesh, oldCamera360, differences, selfDestroy, selfBuild);
}
