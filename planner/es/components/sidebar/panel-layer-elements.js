...
import { MODE_IDLE, MODE_2D_ZOOM_IN, MODE_2D_ZOOM_OUT, MODE_2D_PAN, MODE_3D_VIEW, MODE_3D_FIRST_PERSON, MODE_WAITING_DRAWING_LINE, MODE_DRAWING_LINE, MODE_DRAWING_HOLE, MODE_DRAWING_ITEM, MODE_DRAWING_CAMERA, MODE_DRAWING_CAMERA360, MODE_DRAGGING_LINE, MODE_DRAGGING_VERTEX, MODE_DRAGGING_ITEM, MODE_DRAGGING_CAMERA, MODE_DRAGGING_CAMERA360, MODE_DRAGGING_HOLE, MODE_FITTING_IMAGE, MODE_UPLOADING_IMAGE, MODE_ROTATING_ITEM, MODE_ROTATING_CAMERA, MODE_ROTATING_CAMERA360 } from '../../constants';
...
var VISIBILITY_MODE = {
    ...
    MODE_DRAWING_CAMERA: MODE_DRAWING_CAMERA,
    MODE_DRAWING_CAMERA360: MODE_DRAWING_CAMERA360,
    MODE_DRAGGING_LINE: MODE_DRAGGING_LINE,
    MODE_DRAGGING_VERTEX: MODE_DRAGGING_VERTEX,
    MODE_DRAGGING_ITEM: MODE_DRAGGING_ITEM,
    MODE_DRAGGING_CAMERA: MODE_DRAGGING_CAMERA,
    MODE_DRAGGING_CAMERA360: MODE_DRAGGING_CAMERA360,
    ...
    MODE_ROTATING_CAMERA: MODE_ROTATING_CAMERA,
    MODE_ROTATING_CAMERA360: MODE_ROTATING_CAMERA360
};
...
var PanelLayerElement = function (_Component) {
    ...
    _createClass(PanelLayerElement, [{
        ...
        value: function shouldComponentUpdate(nextProps, nextState) {
            ...
            if (oldElements.lines.hashCode() !== newElements.lines.hashCode() || oldElements.holes.hashCode() !== newElements.holes.hashCode() || oldElements.items.hashCode() !== newElements.items.hashCode() || oldElements.cameras.hashCode() !== newElements.cameras.hashCode()) return true;
            ...
            if (this.state.matchString !== '') {
                ...
                this.setState({
                    matchedElements: {
                        ...
                        cameras: elements.cameras.filter(filterCb)
                    }
                });
            } else {
        ...
        value: function matcharray(text) {
            ...
            this.setState({
                ...
                matchedElements: {
                    ...
                    cameras: this.state.elements.cameras.filter(filterCb)
                }
            });
        }
    }, {
        ...
        value: function render() {
            ...
            return React.createElement(
                ...
                React.createElement(
                    ...
                    this.state.matchedElements.cameras.count() ? React.createElement(
                        'div',
                        null,
                        React.createElement(
                            'p',
                            { style: categoryDividerStyle },
                            this.context.translator.t('Cameras')
                        ),
                        this.state.matchedElements.cameras.entrySeq().map(function (_ref7) {
                            ...
                            return React.createElement(
                                'div',
                                {
                                    key: cameraID,
                                    onClick: function onClick(e) {
                                        return _this2.context.camerasActions.selectCamera(layer.id, camera.id);
                                    },
                                    style: camera.selected ? elementSelectedStyle : elementStyle
                                },
                                camera.name
                            );
                        })
