...
import { MODE_IDLE, MODE_DRAWING_CAMERA360, MODE_DRAGGING_CAMERA360, MODE_ROTATING_CAMERA360, MODE_RESIZING_CAMERA360 } from '../constants';
...
var Camera360 = function () {
  ...
  _createClass(Camera360, null, [{
    ...
    value: function create(state, layerID, type, x, y, rotation) {
      var camera360ID = IDBroker.acquireID();
      console.log('camera360-create');
      var camera360 = state.catalog.factoryElement(type, {
        id: camera360ID,
        name: NameGenerator.generateName('cameras360', state.catalog.getIn(['elements', type, 'info', 'title'])),
        type: type,
        x: x,
        y: y,
        rotation: rotation
    ...
    value: function remove(state, layerID, camera360ID) {
      ...
      state.getIn(['scene', 'groups']).forEach(function (group) {
        return state = Group.removeElement(state, group.id, layerID, 'cameras360', camera360ID).updatedState;
      });
    ...
    value: function updateResizingCamera360(state, x, y) {
      ...
      var mousePositionToCamera360Center = calculatePointFromLineWithAngleAndPoint(originalX, originalY, sideAngle, x, y);
