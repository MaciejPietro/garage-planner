...
import { LOAD_PROJECT, NEW_PROJECT, OPEN_CATALOG, SELECT_TOOL_EDIT, MODE_IDLE, UNSELECT_ALL, SET_PROPERTIES, SET_PROPERTIES_ALL, SET_ITEMS_ATTRIBUTES, SET_CAMERAS_ATTRIBUTES, SET_CAMERAS360_ATTRIBUTES, SET_LINES_ATTRIBUTES, SET_HOLES_ATTRIBUTES, REMOVE, UNDO, ROLLBACK, SET_PROJECT_PROPERTIES, INIT_CATALOG, UPDATE_MOUSE_COORDS, UPDATE_ZOOM_SCALE, TOGGLE_SNAP, CHANGE_CATALOG_PAGE, GO_BACK_TO_CATALOG_PAGE, THROW_ERROR, THROW_WARNING, COPY_PROPERTIES, PASTE_PROPERTIES, PUSH_LAST_SELECTED_CATALOG_ELEMENT_TO_HISTORY, ALTERATE_STATE, SET_MODE, ADD_HORIZONTAL_GUIDE, ADD_VERTICAL_GUIDE, ADD_CIRCULAR_GUIDE, REMOVE_HORIZONTAL_GUIDE, REMOVE_VERTICAL_GUIDE, REMOVE_CIRCULAR_GUIDE, OPEN_PROJECT_CONFIGURATOR } from '../constants';

import { Project, Line } from '../class/export';
...
export default function (state, action) {
    switch (action.type) {
        ...
        case SET_PROPERTIES:
            ...
            // console.log('SET_PROPERTIES_ALL selected', selected);
            return Project.setProperties(state, state.getIn(['scene', 'selectedLayer']), action.properties).updatedState;

        case SET_PROPERTIES_ALL:
            var lines = state.getIn(['scene', 'layers', state.getIn(['scene', 'selectedLayer']), 'lines']).toJS();

            Object.keys(lines).forEach(function (lineID) {
                var properties = state.getIn(['scene', 'layers', state.getIn(['scene', 'selectedLayer']), 'lines', lineID, 'properties']);

                var newProps = properties.set('height', Map().set('length', action.height));

                state = state.setIn(['scene', 'layers', state.getIn(['scene', 'selectedLayer']), 'lines', lineID, 'properties'], newProps);
            });
        ...
        case SET_CAMERAS_ATTRIBUTES:
            state = state.merge({
                sceneHistory: history.historyPush(state.sceneHistory, state.scene)
            });
            return Project.setCamerasAttributes(state, action.camerasAttributes).updatedState;

        case SET_CAMERAS360_ATTRIBUTES:
            state = state.merge({
                sceneHistory: history.historyPush(state.sceneHistory, state.scene)
            });
            return Project.setCameras360Attributes(state, action.cameras360Attributes).updatedState;
