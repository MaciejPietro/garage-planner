...
exports.default = function (state, action) {
  switch (action.type) {
    ...
    case _constants.BEGIN_RESIZING_ITEM:
      state = state.merge({ sceneHistory: _export2.history.historyPush(state.sceneHistory, state.scene) });
      return _export.Item.beginResizingItem(state, action.layerID, action.itemID, action.x, action.y, action.side).updatedState;

    case _constants.UPDATE_RESIZING_ITEM:
      return _export.Item.updateResizingItem(state, action.x, action.y).updatedState;

    case _constants.END_RESIZING_ITEM:
      state = state.merge({ sceneHistory: _export2.history.historyPush(state.sceneHistory, state.scene) });
      return _export.Item.endResizingItem(state, action.x, action.y).updatedState;
