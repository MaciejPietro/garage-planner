        ...
        window.addEventListener('keydown', function (event) {
            ...
            switch (event.keyCode) {
                ...
                case _constants.KEYBOARD_BUTTON_CODE.DELETE:
                    {
                        if ([_constants.MODE_IDLE, _constants.MODE_3D_VIEW].includes(mode)) store.dispatch((0, _projectActions.remove)());
                        break;
                    }
                ...
                case _constants.KEYBOARD_BUTTON_CODE.C:
                    {
                        ...
                        if ((mode === _constants.MODE_IDLE || mode === _constants.MODE_3D_VIEW) && (selected.holes.size || selected.areas.size || selected.items.size || selected.cameras.size || selected.cameras360.size || selected.lines.size)) {
                            ...
                            } else if (selected.cameras.size) {
                                var camera = state.getIn(['scene', 'layers', selectedLayer, 'cameras', selected.cameras.get(0)]);
                                store.dispatch((0, _projectActions.copyProperties)(camera.properties));
                            } else if (selected.cameras360.size) {
                                var camera360 = state.getIn(['scene', 'layers', selectedLayer, 'cameras360', selected.cameras360.get(0)]);
                                store.dispatch((0, _projectActions.copyProperties)(camera360.properties));
