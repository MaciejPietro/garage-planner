    ...
    _createClass(Group, null, [{
        ...
        value: function select(state, groupID) {
            ...
            layerList.entrySeq().forEach(function (_ref) {
                ...
                var cameras = groupLayerElements.get('cameras');
                var cameras360 = groupLayerElements.get('cameras360');
                ...
                if (cameras) cameras.forEach(function (cameraID) {
                    state = _export.Camera.select(state, groupLayerID, cameraID).updatedState;
                });
                if (cameras360) cameras360.forEach(function (camera360ID) {
                    state = _export.Camera360.select(state, groupLayerID, camera360ID).updatedState;
                });
        ...
        value: function createFromSelectedElements(state) {
            ...
            state.getIn(['scene', 'layers']).forEach(function (layer) {
                ...
                var layerElements = {
                    ...
                    cameras: layer.get('cameras').filter(function (el) {
                        return el.get('selected');
                    }),
                    cameras360: layer.get('cameras360').filter(function (el) {
                        return el.get('selected');
                    }),
        ...
        value: function reloadBaricenter(state, groupID) {
            ...
            layerList.entrySeq().forEach(function (_ref3) {
                ...
                if (cameras) cameras.forEach(function (cameraID) {
                    var _state$getIn2 = state.getIn(['scene', 'layers', groupLayerID, 'cameras', cameraID]),
                        x = _state$getIn2.x,
                        y = _state$getIn2.y;
                ...
                if (cameras360) cameras360.forEach(function (camera360ID) {
                    var _state$getIn3 = state.getIn(['scene', 'layers', groupLayerID, 'cameras360', camera360ID]),
                        x = _state$getIn3.x,
                        y = _state$getIn3.y;
        ...
        value: function removeAndDeleteElements(state, groupID) {
            ...
            layerList.entrySeq().forEach(function (_ref5) {
                ...
                if (cameras) cameras.forEach(function (cameraID) {
                    state = _export.Camera.remove(state, groupLayerID, cameraID).updatedState;
                });
                if (cameras360) cameras360.forEach(function (camera360ID) {
                    state = _export.Camera360.remove(state, groupLayerID, camera360ID).updatedState;
                });
        ...
        value: function translate(state, groupID, x, y) {
            ...
            layerList.entrySeq().forEach(function (_ref7) {
                ...
                if (cameras) state = cameras.map(function (cameraID) {
                    return state.getIn(['scene', 'layers', groupLayerID, 'cameras', cameraID]);
                }).reduce(function (newState, camera) {
                    var xI = camera.x,
                        yI = camera.y;

                    return _export.Camera.setAttributes(newState, groupLayerID, camera.id, new _immutable.Map({ x: xI + deltaX, y: yI + deltaY })).updatedState;
                }, state);

                if (cameras360) state = cameras360.map(function (camera360ID) {
                    return state.getIn(['scene', 'layers', groupLayerID, 'cameras360', camera360ID]);
                }).reduce(function (newState, camera360) {
                    var xI = camera360.x,
                        yI = camera360.y;

                    return _export.Camera360.setAttributes(newState, groupLayerID, camera360.id, new _immutable.Map({ x: xI + deltaX, y: yI + deltaY })).updatedState;
                }, state);
        ...
        value: function rotate(state, groupID, newAlpha) {
            var _state$getIn4 = state.getIn(['scene', 'groups', groupID]),
                barX = _state$getIn4.x,
                barY = _state$getIn4.y,
                rotation = _state$getIn4.rotation;
            ...
            layerList.entrySeq().forEach(function (_ref9) {
                ...
                if (cameras) state = cameras.map(function (cameraID) {
                    ...
                    var _GeometryUtils$rotate3 = _export2.GeometryUtils.rotatePointAroundPoint(xI, yI, barX, barY, alpha),
                        newX = _GeometryUtils$rotate3.x,
                        newY = _GeometryUtils$rotate3.y;

                    return _export.Camera.setAttributes(newState, groupLayerID, camera.id, new _immutable.Map({ x: newX, y: newY, rotation: rI + alpha })).updatedState;
                }, state);
                ...
                if (cameras360) state = cameras360.map(function (camera360ID) {
                    ...
                    var xI = camera360.x,
                        yI = camera360.y,
                        rI = camera360.rotation;

                    var _GeometryUtils$rotate4 = _export2.GeometryUtils.rotatePointAroundPoint(xI, yI, barX, barY, alpha),
                        newX = _GeometryUtils$rotate4.x,
                        newY = _GeometryUtils$rotate4.y;

                    return _export.Camera360.setAttributes(newState, groupLayerID, camera360.id, new _immutable.Map({ x: newX, y: newY, rotation: rI + alpha })).updatedState;
                }, state);
