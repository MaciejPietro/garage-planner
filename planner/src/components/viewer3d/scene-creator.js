...
function createLayerObjects(layer, planData, sceneData, actions, catalog) {
    ...
    // Import cameras
    layer.cameras360.forEach((camera360) => {
        promises.push(
            addCamera360(
                sceneData,
                planData,
        ...
        if (path[1] === 'layers') {
            ...
            if (layerGraph) {
                if (!layerGraph.visible) {
                    ...
                    for (let cameraID in layerGraph.cameras)
                        removeCamera(planData, layerGraph.id, cameraID);
                    for (let camera360ID in layerGraph.cameras360)
...
function replaceObject(
    ...
    switch (modifiedPath[3]) {
        ...
        case 'items':
            ...
            break;
        case 'cameras':
            let camera = layer.getIn(['cameras', modifiedPath[4]]);
        ...
        case 'cameras360':
            let camera360 = layer.getIn(['cameras360', modifiedPath[4]]);
...
function removeLayer(layerId, planData) {
    ...
    for (let cameraID in layerGraph.cameras) removeCamera(planData, layerId, cameraID);
    for (let camera360ID in layerGraph.cameras360)
...
function removeCamera(planData, layerId, cameraID) {
    if (planData.sceneGraph.busyResources.layers[layerId].cameras[cameraID]) {
        setTimeout(() => removeCamera(planData, layerId, cameraID), 100);
        return;
    }

    planData.sceneGraph.busyResources.layers[layerId].cameras[cameraID] = true;

    let camera3D = planData.sceneGraph.layers[layerId].cameras[cameraID];
...
function removeCamera360(planData, layerId, camera360ID) {
    if (planData.sceneGraph.busyResources.layers[layerId].cameras360[camera360ID]) {
        setTimeout(() => removeCamera360(planData, layerId, camera360ID), 100);
        return;
    }

    planData.sceneGraph.busyResources.layers[layerId].cameras360[camera360ID] = true;

    let camera3603D = planData.sceneGraph.layers[layerId].cameras360[camera360ID];
...
function addCamera(sceneData, planData, layer, cameraID, catalog, camerasActions) {
    let camera = layer.getIn(['cameras', cameraID]);
...
function addCamera360(
    ...
    let camera360 = layer.getIn(['cameras360', camera360ID]);
...
function updateCamera(
    ...
    let camera = layer.getIn(['cameras', cameraID]);
    let oldCamera = oldSceneData.getIn(['layers', layer.id, 'cameras', cameraID]);
    let mesh = planData.sceneGraph.layers[layer.id].cameras[cameraID];
...
function updateCamera360(
    ...
    let camera360 = layer.getIn(['cameras360', camera360ID]);
    let oldCamera360 = oldSceneData.getIn([
        'layers',
        layer.id,
        'cameras360',
        camera360ID,
    ]);
    let mesh = planData.sceneGraph.layers[layer.id].cameras360[camera360ID];
